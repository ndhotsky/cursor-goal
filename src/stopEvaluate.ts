...
export function parseStopHookInput(raw: unknown): StopHookInput {
  ...
  return {
    ...
    generation_id: optionalNonEmptyString(input.generation_id),
    status,
...
export async function evaluateStopHook(
  ...
  if (input.loop_count >= state.budgets.maxTurns) {
    await finalizeStopEvaluation({
      ...
      markdown: [
        `Stop hook loop_count ${input.loop_count} reached max turns ${state.budgets.maxTurns}.`,
        "",
        formatStopHookProvenance(input),
      ].join("\n"),
    })
    return {}
  }
...
function formatStopHookProvenance(input: StopHookInput) {
  return [
    `Conversation ID: ${formatHookValue(input.conversation_id)}`,
    `Generation ID: ${formatHookValue(input.generation_id)}`,
    `Transcript path: ${formatTranscriptPath(input.transcript_path)}`,
  ].join("\n")
}

function formatHookValue(value?: string) {
  return value ? clipInline(value, 200) : "not provided"
}
