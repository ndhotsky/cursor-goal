...
if [[ "$GLOBAL" == true ]]; then
  ...
  HOOK_PROJECT_ROOT="${TARGET:-$PWD}"
fi

HOOKS_FILE="$HOOKS_FILE" HOOK_PROJECT_ROOT="$HOOK_PROJECT_ROOT" EVALUATE_GOAL_SCRIPT="$EVALUATE_GOAL_SCRIPT" node <<'NODE'
const fs = require("node:fs")
const path = require("node:path")

const hooksFile = process.env.HOOKS_FILE
const resolvedCommand = process.env.EVALUATE_GOAL_SCRIPT
const projectRoot = process.env.HOOK_PROJECT_ROOT

if (!hooksFile || !resolvedCommand) {
  console.error("error: missing hooks file or evaluate-goal script path")
  process.exit(1)
}

const command = projectRoot ? relativeHookCommand(resolvedCommand, projectRoot) : resolvedCommand

function relativeHookCommand(command, root) {
  const relative = path.relative(path.resolve(root), path.resolve(command))
  return relative && !relative.startsWith("..") && !path.isAbsolute(relative) ? relative : command
}
