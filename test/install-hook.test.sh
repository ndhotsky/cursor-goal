...
if (entry.type !== "command") {
  throw new Error("expected command hook type")
}
...
"${BIN}/cursor-goal-install-hook" "${PKG}"

test -f "${PKG}/.cursor/hooks.json"
node - "${PKG}/.cursor/hooks.json" "scripts/evaluate-goal.sh" <<'NODE'
const fs = require("node:fs")
...
if (entry?.command !== expectedCommand) {
  throw new Error(`unexpected project hook command: ${entry?.command}`)
}
if (entry.type !== "command") {
  throw new Error("expected project command hook type")
}
NODE
