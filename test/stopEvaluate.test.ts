...
test("evaluateStopHook continues when verification fails", async () => {
  ...
  const runLog = await fs.readFile(state.runLogPath, "utf8")
  assert.match(runLog, /Conversation ID: conv-fail/)
  assert.match(runLog, /Generation ID: gen-fail/)
  assert.match(runLog, /Transcript path: present/)
  assert.match(runLog, /transcript\.jsonl/)
  if (process.env.HOME) assert.ok(!runLog.includes(process.env.HOME))
})
